...
/**
 * BasicPlayer is a threaded simple player class based on JavaSound API. It has been successfully
 * tested under J2SE 1.3.x, 1.4.x and 1.5.x.
 */
public class BasicPlayer implements BasicController, Runnable {
  ...
  private static Logger log = Logger.getLogger("ILoveRadio");
    ...
    try {
      ...
      Map properties = null;
        if (m_audioFileFormat instanceof TAudioFileFormat) {
            ...
            properties = m_audioFileFormat.properties();
      ...
      if (audioFormat instanceof TAudioFormat) {
        ...
        Map addproperties = audioFormat.properties();
   ...
   * <p>
   * From the AudioInputStream, i.e. from the sound file, we fetch information about the format of
   * the audio data. These information include the sampling frequency, the number of channels and
  ...
  protected void createLine() throws LineUnavailableException {
    ...
    if (m_line == null) {
      ...
      int nSampleSizeInBits = sourceFormat.getSampleSizeInBits();
        ...
        if ((sourceFormat.getEncoding() == AudioFormat.Encoding.ULAW) || (sourceFormat.getEncoding()
      ...
      AudioFormat targetFormat = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED,
          ...
          sourceFormat.getChannels() * (nSampleSizeInBits / 8), sourceFormat.getSampleRate(),
          false);
      ...
      log.fine("Line Info : " + m_line.getLineInfo().toString());
      log.fine("Line AudioFormat: " + m_line.getFormat().toString());
    }
  }
  ...
  protected void openLine() throws LineUnavailableException {
    if (m_line != null) {
      ...
      for (int p = 0; p < c.length; p++) {
        log.fine("Controls : " + c[p].toString());
      }
      ...
      if (m_line.isControlSupported(FloatControl.Type.PAN)) {
        ...
        log.info(
            "Pan Control : [" + m_panControl.getMinimum() + "," + m_panControl.getMaximum() + "] "
   ...
   * <p>
   * Player Status = STOPPED.<br> Thread should free Audio ressources.
   */
   ...
   * Main loop.
   * <p>
   * Player Status == STOPPED || SEEKING => End of Thread + Freeing Audio Ressources.<br> Player
   * Status == PLAYING => Audio stream data sent to Audio line.<br> Player Status == PAUSED =>
   * Waiting for another status.
   */
  public void run() {
    ...
    synchronized (m_audioInputStream) {
      ...
      while ((nBytesRead != -1) && (m_status != STOPPED) && (m_status != SEEKING) && (m_status
          != UNKNOWN)) {
        if (m_status == PLAYING) {
          // Play.
          try {
            ...
            if (nBytesRead >= 0) {
              ...
              System.arraycopy(abData, 0, pcm, 0, nBytesRead);
                if (m_line.available() >= m_line.getBufferSize()) {
                    log.fine("Underrun : " + m_line.available() + "/" + m_line.getBufferSize());
                }
          ...
          try {
            ...
            log.log(Level.SEVERE, "Thread cannot sleep(1000)", e);
          }
        }
      }
  ...
  protected void notifyEvent(int code, int position, double value, Object description) {
    BasicPlayerEventLauncher trigger = new BasicPlayerEventLauncher(code, position, value,
        description, new ArrayList(m_listeners), this);
    trigger.start();
  }
  ...
  /**
   * Sets Pan value. Line should be opened before calling this method. Linear scale : -1.0 <-->
   * +1.0
   */
  public void setPan(double fPan) throws BasicPlayerException {
      if (hasPanControl()) {
          log.log(Level.SEVERE, "Pan : " + fPan);
